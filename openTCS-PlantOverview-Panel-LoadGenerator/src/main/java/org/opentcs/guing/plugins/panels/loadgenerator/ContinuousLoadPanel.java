...
public class ContinuousLoadPanel
  ...
  /**
   * Provides access to a portal.提供对门户的访问。
   */
  ...
  /**
   * The instance trigger creation of new orders.实例触发新订单的创建。
   */
  ...
  public void terminate() {
    ...
    System.out.println("Disable order generation!");
  ...
  private OrderBatchCreator createOrderBatchCreator() {
    if (randomOrderSpecButton.isSelected()) {//随机创建
      int orderCount = (Integer) randomOrderCountSpinner.getValue();//批次个数
      int orderSize = (Integer) randomOrderSizeSpinner.getValue();//driverOrder数量
      //返回随机批次订单
    ...
    else if (explicitOrderSpecButton.isSelected()) {//明确的顺序创建订单
      saveCurrentTableData();
      TransportOrderTableModel tableModel
          = (TransportOrderTableModel) toTable.getModel();//获取面板的数据
        ...
        if (curData.getDriveOrders().isEmpty()) {
          ...
          for (DriveOrderStructure curDOS : curData.getDriveOrders()) {
            ...
            if (curDOS.getDriveOrderLocation() == null
              ...
              System.out.println("判断出Location 和 Operation为空");
              return null;
            }
          }
        }
  ...
  private OrderGenerationTrigger createOrderGenTrigger() {
    ...
    if (thresholdTriggerRadioButton.isSelected()) {
      System.out.println("如果正在处理的传输订单数量降至或低于给定阈值，则触发一批订单的创建");
      ...
      System.out.println("创建在给定超时后触发一批订单。");
  ...
  private void initComponents() {
    ...
    MyOrderGenChkBox = new javax.swing.JCheckBox();
    ...
    MyOrderGenChkBox.setText(bundle.getString("continuousLoadPanel.checkBox_enableMyOrderGeneration.text")); // NOI18N
    MyOrderGenChkBox.addItemListener(new java.awt.event.ItemListener() {
      public void itemStateChanged(java.awt.event.ItemEvent evt) {
        MyOrderGenChkBoxItemStateChanged(evt);//触发是否生成持续的订单
      }
    });
    gridBagConstraints = new java.awt.GridBagConstraints();
    gridBagConstraints.gridx = 2;
//    gridBagConstraints.gridy = 2;
    gridBagConstraints.fill = java.awt.GridBagConstraints.HORIZONTAL;
    orderGenPanel.add(MyOrderGenChkBox, gridBagConstraints);
  ...
  private javax.swing.JCheckBox MyOrderGenChkBox;
