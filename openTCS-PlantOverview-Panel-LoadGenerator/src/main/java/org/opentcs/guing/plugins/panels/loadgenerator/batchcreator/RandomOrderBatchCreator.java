...
public class RandomOrderBatchCreator
  ...
  public RandomOrderBatchCreator(TransportOrderService transportOrderService,
    ...
    locations = new ArrayList<>(transportOrderService.fetchObjects(Location.class));//获取地图Location数据
  }

  @Override
  public Set<TransportOrder> createOrderBatch()
      throws KernelRuntimeException {
    Set<TransportOrder> createdOrders = new HashSet<>();//订单列表
    if (locations.isEmpty()) {
      return createdOrders;
    }
    for (int i = 0; i < batchSize; i++) {
      createdOrders.add(createSingleOrder());//添加几个生成的订单
    }
  ...
  private TransportOrder createSingleOrder()//生成订单
      throws KernelRuntimeException {
    List<DestinationCreationTO> dests = new ArrayList<>();//目的地列表
    for (int j = 0; j < orderSize; j++) {
      Location destLoc = locations.get(random.nextInt(locations.size()));//随机几次获取Location
      dests.add(new DestinationCreationTO(destLoc.getName(), Destination.OP_NOP));//到目的地无操作
    }
